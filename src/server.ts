...
export function createUnoServer(options: { maxRooms?: number } = {}) {
  ...
  io.on("connection", (socket: Socket) => {
    ...
    bind("c_create_room", z.object({ displayName: name, avatar }).strict(), ({ displayName, avatar: av }) => {
      ...
      const room = new GameRoom(newCode()); rooms.set(room.state.roomCode, room); attach(room, displayName, av);
    });
    bind("c_create_bot_room", z.object({ displayName: name, avatar }).strict(), ({ displayName, avatar: av }) => {
      free(); if (rooms.size >= (options.maxRooms ?? 1000)) throw Error("সার্ভার এখন ভর্তি। একটু পরে চেষ্টা করো।");
      const room = new GameRoom(newCode()); rooms.set(room.state.roomCode, room);
    ...
    bind("c_join_room", z.object({ roomCode: code, displayName: name, avatar }).strict(), ({ roomCode, displayName, avatar: av }) => {
      free(); const room = rooms.get(roomCode); if (!room) throw Error("এই রুমটা পাওয়া গেল না।"); attach(room, displayName, av);
    });
    ...
    // No free-form chat. Fast Troll is a curated, transient room-wide board reaction.
  ...
  const ticker = setInterval(() => {
    for (const [code,room] of rooms) {
      ...
      if(abandoned||idle){ for(const t of room.tokens){const id=room.player(t).socketId; membership.delete(id); io.to(id).emit("s_session_expired",{});} rooms.delete(code); }
    }
