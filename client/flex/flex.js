(() => {
  ...
  let avatar = localStorage.getItem('flex-avatar') || '😎';
  let state = null;
  let saved = null;
  ...
  const tutorials = [
    ...
    { icon:'🎨', title:'Regular side vs FLEX side', body:'A regular side plays normally. A FLEX side can match using its secondary color when your Power is ON. Using the FLEX side does not permanently replace the active color unless the card is Wild.', demo:'flex' },
    ...
    { icon:'🚨', title:'UNO and winning', body:'When you reach one card, press UNO before somebody catches you. If caught, draw 2. First player to empty their hand wins the round.', demo:'uno' }
  ];
  ...
  function tutorialDemo(type){
    ...
    if(type==='uno') return '<div class="card-demo"><div class="mini-card red">UNO!</div></div>';
  ...
  function renderTutorial(){
    ...
    $('tutorial-next').textContent = tutorialIndex === tutorials.length-1 ? 'Choose mode ⚡' : 'Next →';
  }

  $('tutorial-back').onclick = () => { if(tutorialIndex>0){tutorialIndex--;renderTutorial();} };
  $('tutorial-next').onclick = () => { if(tutorialIndex < tutorials.length-1){tutorialIndex++;renderTutorial();return;} localStorage.setItem('flex-tutorial-seen','1'); if(selectedMode){openSetup(selectedMode);} else show('mode'); };
  ...
  socket.on('connect',()=>{ if(saved?.roomCode && saved?.token) socket.emit('f_reconnect',{roomCode:saved.roomCode,sessionToken:saved.token}); });
  ...
  socket.on('f_left',()=>{state=null;saved=null;localStorage.removeItem('flex-session');show('mode');});
  ...
  function renderState(){
    ...
    $('status').textContent=state.status==='FINISHED' ? `${state.players.find(p=>p.token===state.winner)?.name||'Player'} WINS 🏆` : state.currentToken===saved?.token ? 'YOUR TURN ⚡' : `${state.players.find(p=>p.token===state.currentToken)?.name||''}'s turn`;
    $('draw').disabled=state.currentToken!==saved?.token || !!state.pendingDraw4 || state.status!=='PLAYING';
  }
